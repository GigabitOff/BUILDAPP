...
**Мультиязычность (ru / uk / en):** см. отдельную карту `I18N_MAP_FOR_CHATGPT.md` в корне репозитория.
...
2. Для задач локализации — дополнительно прочитать `I18N_MAP_FOR_CHATGPT.md`.
3. Определить экран (`lib/screens/**`) и связанный сервис (`lib/services/**`).
4. Проверить API-контракт в разделе endpoints выше перед изменениями.
5. Не менять ключи `SharedPreferences` и не ломать auth-flow без явного запроса.
6. При изменениях в одном модуле сверять модель (`lib/models/**`) и JSON-поля.
