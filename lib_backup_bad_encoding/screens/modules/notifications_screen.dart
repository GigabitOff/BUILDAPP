...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> markAsRead(ObjectNotification notification) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("РћС€РёР±РєР°: ${e.toString().replaceFirst('Exception: ', '')}")),
      );
    }
  }
  ...
  Future<void> markAllAsRead() async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Р’СЃРµ СѓРІРµРґРѕРјР»РµРЅРёСЏ РѕС‚РјРµС‡РµРЅС‹ РїСЂРѕС‡РёС‚Р°РЅРЅС‹РјРё')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        if ((item.actorName ?? '').isNotEmpty)
                          _Badge(text: 'РђРІС‚РѕСЂ: ${item.actorName!}'),
      ...
      return RefreshIndicator(
        ...
        child: ListView(
          ...
          children: const [
            ...
            Center(
              child: Text(
                'РЈРІРµРґРѕРјР»РµРЅРёР№ РїРѕРєР° РЅРµС‚',
            ...
            Center(
              child: Text(
                'РљРѕРіРґР° РїРѕ РѕР±СЉРµРєС‚Р°Рј Р±СѓРґСѓС‚ РёР·РјРµРЅРµРЅРёСЏ, РѕРЅРё РїРѕСЏРІСЏС‚СЃСЏ Р·РґРµСЃСЊ.',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: onlyUnread ? 'РџРѕРєР°Р·Р°С‚СЊ РІСЃРµ' : 'РўРѕР»СЊРєРѕ РЅРµРїСЂРѕС‡РёС‚Р°РЅРЅС‹Рµ',
            onPressed: () {
              setState(() {
                onlyUnread = !onlyUnread;
              });
          ...
          IconButton(
            tooltip: 'РћС‚РјРµС‚РёС‚СЊ РІСЃРµ РїСЂРѕС‡РёС‚Р°РЅРЅС‹РјРё',
