...
class _ObjectTasksScreenState extends State<ObjectTasksScreen> {
  ...
  Future<void> loadTasks() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && data['success'] == true) {
          ...
          tasks.addAll(
            items.map<Map<String, String>>((item) {
              return {
                ...
                'status': '${item['status'] ?? 'РџР»Р°РЅРёСЂСѓРµС‚СЃСЏ'}',
        ...
        throw Exception(data['message'] ?? 'РћС€РёР±РєР° Р·Р°РіСЂСѓР·РєРё Р·Р°РґР°С‡');
      }
    } catch (e) {
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'РќРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РіСЂСѓР·РёС‚СЊ Р·Р°РґР°С‡Рё: ${e.toString().replaceFirst('Exception: ', '')}',
          ),
        ),
      );
    } finally {
  ...
  Future<bool> createTask(String status) async {
    ...
    if (title.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Р’РІРµРґРё РЅР°Р·РІР°РЅРёРµ Р·Р°РґР°С‡Рё')));
      return false;
    }
    ...
    try {
      ...
      if ((response.statusCode == 200 || response.statusCode == 201) &&
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Р—Р°РґР°С‡Р° СЃРѕР·РґР°РЅР°')));

        return true;
      } else {
        throw Exception(data['message'] ?? 'РћС€РёР±РєР° СЃРѕР·РґР°РЅРёСЏ Р·Р°РґР°С‡Рё');
      }
    } catch (e) {
      if (!mounted) return false;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'РќРµ СѓРґР°Р»РѕСЃСЊ СЃРѕР·РґР°С‚СЊ Р·Р°РґР°С‡Сѓ: ${e.toString().replaceFirst('Exception: ', '')}',
          ),
        ),
      );
  ...
  Future<void> deleteTask(String taskId) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && data['success'] == true) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Р—Р°РґР°С‡Р° СѓРґР°Р»РµРЅР°')));
      } else {
        throw Exception(data['message'] ?? 'РћС€РёР±РєР° СѓРґР°Р»РµРЅРёСЏ Р·Р°РґР°С‡Рё');
      }
    } catch (e) {
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'РќРµ СѓРґР°Р»РѕСЃСЊ СѓРґР°Р»РёС‚СЊ Р·Р°РґР°С‡Сѓ: ${e.toString().replaceFirst('Exception: ', '')}',
          ),
        ),
      );
    }
  ...
  void showAddTaskSheet() {
    if (!isAdmin) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('РўРѕР»СЊРєРѕ Р°РґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂ РјРѕР¶РµС‚ СЃРѕР·РґР°РІР°С‚СЊ Р·Р°РґР°С‡Рё'),
        ),
      );
      return;
    }
    ...
    String selectedStatus = 'РџР»Р°РЅРёСЂСѓРµС‚СЃСЏ';
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: const InputDecoration(
                            labelText: 'РќР°Р·РІР°РЅРёРµ Р·Р°РґР°С‡Рё',
                            hintText: 'РќР°РїСЂРёРјРµСЂ: РїСЂРѕРІРµСЂРёС‚СЊ РјРѕРЅС‚Р°Р¶ РѕРєРѕРЅ',
                        ...
                        TextField(
                          ...
                          decoration: const InputDecoration(
                            ...
                            hintText: 'Р§С‚Рѕ РЅСѓР¶РЅРѕ СЃРґРµР»Р°С‚СЊ?',
                            border: OutlineInputBorder(),
                          ),
                        ),
                          ...
                          children: [
                            ...
                            Expanded(
                              child: ElevatedButton.icon(
                                ...
                                label: Text(
                                  isSaving ? 'РЎРѕС…СЂР°РЅСЏРµРј...' : 'Р”РѕР±Р°РІРёС‚СЊ',
                                ),
                              ),
                            ),
                          ],
  ...
  void showChangeStatusSheet(Map<String, String> task) {
    ...
    String selectedStatus = task['status'] ?? 'РџР»Р°РЅРёСЂСѓРµС‚СЃСЏ';
                      ...
                      children: [
                        ...
                        Text(
                          (task['description'] ?? '').isEmpty
                              ? 'РћРїРёСЃР°РЅРёРµ РЅРµ СѓРєР°Р·Р°РЅРѕ'
                              : task['description'] ?? '',
                          style: const TextStyle(
                            fontSize: 14,
                            color: Colors.black54,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              isAdmin
                  ? 'РќР°Р¶РјРё вЂњР”РѕР±Р°РІРёС‚СЊвЂќ, С‡С‚РѕР±С‹ СЃРѕР·РґР°С‚СЊ РїРµСЂРІСѓСЋ Р·Р°РґР°С‡Сѓ РїРѕ РѕР±СЉРµРєС‚Сѓ.'
                  : 'РџРѕ СЌС‚РѕРјСѓ РѕР±СЉРµРєС‚Сѓ Р·Р°РґР°С‡ РїРѕРєР° РЅРµС‚.',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text(
          'Р—Р°РґР°С‡Рё РїРѕ РѕР±СЉРµРєС‚Сѓ',
      ...
      floatingActionButton: isAdmin
          ? FloatingActionButton.extended(
              ...
              label: Text(isSaving ? 'РЎРѕС…СЂР°РЅСЏРµРј...' : 'Р”РѕР±Р°РІРёС‚СЊ'),
            )
          : null,
      body: isLoading
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    title.isEmpty ? 'Р‘РµР· РЅР°Р·РІР°РЅРёСЏ' : title,
                    style: const TextStyle(
                      fontSize: 17,
                      fontWeight: FontWeight.w900,
                    ),
...
class _TaskInfoRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final safeValue = value.isEmpty ? 'РќРµ СѓРєР°Р·Р°РЅРѕ' : value;
