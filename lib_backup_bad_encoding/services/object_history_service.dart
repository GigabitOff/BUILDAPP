...
class ObjectHistoryService {
  ...
  static Future<List<ObjectHistoryItem>> getHistory(int objectId) async {
    ...
    if (response.statusCode != 200 || data['success'] != true) {
      throw Exception(data['message'] ?? 'РћС€РёР±РєР° РїРѕР»СѓС‡РµРЅРёСЏ РёСЃС‚РѕСЂРёРё РѕР±СЉРµРєС‚Р°');
    }
  ...
  static Future<ObjectHistoryItem> createHistoryItem({
    ...
    if ((response.statusCode != 200 && response.statusCode != 201) ||
        data['success'] != true) {
      throw Exception(data['message'] ?? 'РћС€РёР±РєР° РґРѕР±Р°РІР»РµРЅРёСЏ Р·Р°РїРёСЃРё РёСЃС‚РѕСЂРёРё');
    }
