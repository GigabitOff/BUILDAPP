...
class _ObjectMaterialsScreenState extends State<ObjectMaterialsScreen> {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text('Удалить "${material.name}"?'),
          actions: [
            TextButton(
  ...
  Future<void> showMaterialDialog({ObjectMaterial? material}) async {
        ...
        return StatefulBuilder(
          builder: (context, setSheetState) {
            Future<void> save() async {
              ...
              if (name.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Введи название материала')),
                );
                return;
              }
    ...
    if (saved == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isEdit ? 'Материал обновлён' : 'Материал добавлен'),
        ),
      );
    }
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Кол-во: ${material.quantity} ${material.unit ?? ''}',
                  style: const TextStyle(color: Colors.black54),
                ),
                Text(
                  'Цена: ${money(material.price)} грн',
                  style: const TextStyle(color: Colors.black54),
                ),
                Text(
                  'Сумма: ${money(sum)} грн',
